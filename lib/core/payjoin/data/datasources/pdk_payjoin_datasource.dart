...
class PdkPayjoinDatasource {
  ...
  // Per-session polling timers, keyed by session id. Polling runs on the
  // main isolate because uniffi-dart callback dispatch is isolate-local —
  // see payjoin-bbm/payjoin_isolates.md.
  ...
  Future<void> _pollReceiverOnce(
    ...
    try {
      ...
      final pj.ReceiveSession state;
      try {
        state = pj.replayReceiverEventLog(persister: persister).state();
      ...
      // Stop polling first so a slow continuation can't be re-entered.
      timer.cancel();
  ...
  Future<void> _pollSenderOnce(
    ...
    try {
      ...
      final pj.SendSession state;
      try {
        state = pj.replaySenderEventLog(persister: persister).state();
  ...
  Future<pj.UncheckedOriginalPayload?> _getUncheckedOriginalPayload(
